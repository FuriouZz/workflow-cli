...
module.exports = {
  ...
  task() {
    ...
    options = options || {}
    action  = action  || function( complete ) {
      if (options.async) complete()
    }
   ...
   */
  command() {
    let name, prerequisites, options, cmd
    name = arguments[0]
  ...
  namespace(name, closure) {
    ...
    if (ns.tasks.default) {
      ...
      tsk._name                  = ns.name
      tsk.path                   = tsk.path.replace(':default', '')
      tsk.description            = tsk.description || ''
      tsk.description            = tsk.description + ' (default)'
      ns.parent.tasks[tsk._name] =  tsk
      delete ns.tasks.default
    }
