...
class PublishTask extends ExtraTask {
  ...
  _configure() {
    ...
    task('bump', { async: true }, function() {
      ...
      serie(tasks).catch(this.fail).done(this.complete)
    ...
    task('bump_version', { visible: true }, function() {
      const nextVersion = wk.Tasks[nm('next_version')].value
     ...
     */
    command('pre_push', 'git ls-remote', function(err, stdout) {
      ...
      wk.ARGV[ nm('_push') ] = { remote: remote, version: version }
     ...
     */
    command('_push', 'git push ${remote} --tag v${version}', {
      visible: false,
