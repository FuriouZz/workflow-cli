...
class PublishTask extends ExtraTask {
  ...
  _configure() {
    ...
    task('next_version', { visible: false }, function() {
      const identifier = wk.ARGV['identifier'] ? wk.ARGV['identifier'] : null
      let release      = typeof wk.ARGV['release'] === 'string' ? wk.ARGV['release'] : 'patch'

      if (wk.ARGV['prerelease'] || wk.ARGV['pre']) release = 'prerelease'
     ...
     */
    command('version_released', 'git ls-remote', function(err, stdout) {
    ...
    task('git_push', { visible: false, async: true }, function() {
      ...
      wk.exec(`git push ${remote} --tag v${version}`).catch(this.fail).then(this.complete)
    })
