...
module.exports = function() {
  ...
  const TASKS = TaskManager._visible(ARGV._)

  if (TASKS.length > 0) {

    // --parallel -p
    if (PARAMS.parallel) {
      parallel( TASKS ).catch(_handleError)
      return
    }
    ...
    if (PARAMS.sequence === 'parallel') {
      ...
      serie( TASKS ).catch(_handleError)
    }

    return
  }

  if (!wk.ARGV['no-help']) help()
