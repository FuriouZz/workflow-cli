...
class Task {
  ...
  execute() {
    ...
    if (!this.action) {
      Print.debug('No action. Complete ' + Print.magenta(`[${this.path}]`))
      this.complete( true )
      return this.promise
    }
  ...
  invoke() {
    ...
    if (!Task.STATUS.IS_PENDING(this.status)) return this.promise
  ...
  fail(value) {
    if (this.status !== Task.STATUS.PROCESS) return this.promise

    this.status = Task.STATUS.FAIL

    this.reject( new Error(value) )
    this.resolve = null
    this.reject  = null
  ...
  _link() {
    if (this.namespace.tasks[this._name]) {
      throw new Error(`Task with name ${this._name} already exists!`)
    }

    this.namespace.tasks[this._name] = this
    this.updatePath()
    wk.Tasks[this.path] = this
  }

  _unlink() {
    if (wk.Tasks[this.path] === this) delete wk.Tasks[this.path]
    if (this.namespace.tasks[this._name] === this) delete this.namespace.tasks[this._name]
  }
