...
task('release', [], function( complete, parameters ) {

  if (versions.indexOf(parameters[0]) === -1) {
    console.log( `Valid arguments: ${ versions.join(', ') }` )
    return false
  }

  wk.run('bump:'+parameters[0]).then(function() {
    console.log('DONE')
  })
  ...
  // execCommands(cmds, function() {
  //   console.log(`Version ${version} bumped and released !`)
  //   complete()
  // })
