...
namespace('check', function() {
  ...
  task('branch', { async: true, visible: false }, function() {
    ...
    exec('git symbolic-ref --short HEAD', function(err, stdout, stderr) {
      ...
      if (stdout.replace(/\n/, '') !== cfg.developBranch) {
        fail(new Error('[Check] The current branch must be "'+ cfg.developBranch +'"'));
        return scope.complete(false)
      }

      console.log('[Check] Current develop branch:', stdout.replace(/\n/, ''))
      scope.complete(true)
    })
  })
  ...
  task('stage', { async: true, visible: false }, function() {
    ...
    exec('git status --porcelain --untracked-files=no', function(err, stdout, stderr) {
      ...
      if (stdout.length !== 0) {
        fail(new Error('[Check] Stage is not clean.'))
        return scope.complete(false)
      }

      console.log('[Check] Stage is clean')
      scope.complete(true)
    })
  })
  ...
  task('next_tag', { async: true, visible: true }, function() {
    ...
    if (!target) {
      console.log(`[Check] Invalid target. Required --target=[${versions.join('|')}]`)
      return false
    }
    ...
    exec('git ls-remote', function(err, stdout, stderr) {
      ...
      if (stdout.match(new RegExp(`refs/tags/v${version}`))) {
        fail(`[Check] Version v${version} already released!`)
        scope.complete( false )
      } else {
        console.log(`[Check] Next version for ${target} bump:`, version)
  ...
  task('current_tag', { async: true, visible: true }, function() {
    ...
    exec('git ls-remote', function(err, stdout, stderr) {
      ...
      if (stdout.match(new RegExp(`refs/tags/v${version}`))) {
        console.log(`[Check] Version v${version} already released!`)
        scope.complete( false )
      } else {
        console.log(`[Check] Ready to release:`, version)
        scope.complete( version )
      }
...
task('commit', { async: true, visible: false }, function() {
  ...
  execCommands(cmds, function() {
    console.log(`[Release] Version ${version} bumped !`)
    scope.complete()
  })
...
task('push', ['release:check', 'release:check:current_tag'], function() {
  ...
  if (!version) fail(`[Release] Version invalid: ${version}`)

  exec(`git push ${cfg.remote} v${version}`, function(err, stdout, stderr) {
    if (err) console.log(err)
    else {
      console.log(`[Release] Version v${version} released!`)
    }
  })
