...
class Parser {
  ...
  _getKeyValue(parameters) {
    ...
    for (let i = 0, len = params.length; i < len; i++) {
      ...
      // Special condition for task arguments (must use double quote)
      // eg.: wk "message[ foo bar baz ]"
      else if (str && str.match(/[\w:]+\[.+\]/)) {
        let s     = str.split(/\[|\]/).join('').trim()
        s         = s.split(' ')
        key       = s.shift()
        value     = this._getKeyValue( s )
        params[i] = key
      }
