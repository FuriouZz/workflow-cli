...
class Parser {
  ...
  splitParameters( parameters ) {
    ...
    for (let i = 0, len = args.length; i < len; i++) {
      ...
      if (parameterIndex < 0) {
        ...
        for (const key in this.config) {
          ...
          if (!match && this.config[key].aliases) {
            for (let j = 0, leng = this.config[key].aliases.length; j < leng; j++) {
              match = parameter.match(new RegExp('^'+this.config[key].aliases[j]+'$'))
              if (match) break
            }
          }

          if (match && this.config[key].type === 'boolean') {
            isTask = true
            break
          }
        }
    ...
    return { wkParameters: parameters, taskParameters: [] }
...
const ParserSingleton = new Parser(config)
